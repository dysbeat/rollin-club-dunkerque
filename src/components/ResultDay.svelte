<script>
  ...
  function MakeInfo(x) {
    return x.date + (x.place ? " " + x.place.toLowerCase() : "");
  }
</script>
...
<style>
  ...
  .header {
    display: grid;
    grid-template-columns: auto 300px 50px 10px 50px 300px 100px auto;
...
{#each day as result}
  <div class="results">
    ...
    <p
      class="gray score a {+result.ascore > +result.bscore ? 'highlight' : ''}">
      {result.ascore}
    </p>
    <p
      class="gray score b {+result.bscore > +result.ascore ? 'highlight' : ''}">
      {result.bscore}
    </p>
    <p
