var app = (function () {
	...
	const seasons = readable(seasons_data);

	const selectSeason = derived(
	...
	/* src/components/Season.svelte generated by Svelte v3.0.0 */

	const file$c = "src/components/Season.svelte";

	function create_fragment$c(ctx) {
		var div0, p0, t1, t2, div1, p1, t4, current;
		...
		var results = new Results({
			props: { results: ctx.season.results },
			$$inline: true
		});
		...
		return {
			c: function create() {
				...
				t4 = space();
				results.$$.fragment.c();
				p0.className = "large blue title pad";
				add_location(p0, file$c, 1, 1, 23);
				div0.className = "ranking svelte-1pgna39";
				add_location(div0, file$c, 0, 0, 0);
				p1.className = "large blue title pad";
				add_location(p1, file$c, 5, 1, 125);
				add_location(div1, file$c, 4, 0, 118);
			},
			...
			p: function update(changed, ctx) {
				...
				if (changed.season) rankings_changes.rankings = ctx.season.rankings;
				rankings.$set(rankings_changes);

				var results_changes = {};
				if (changed.season) results_changes.results = ctx.season.results;
	...
	function instance$b($$self, $$props, $$invalidate) {
	  ...
	  let { season } = $$props;
		...
		$$self.$set = $$props => {
			if ('season' in $$props) $$invalidate('season', season = $$props.season);
		};
	...
	class Season extends SvelteComponentDev {
		constructor(options) {
			super(options);
			init(this, options, instance$b, create_fragment$c, safe_not_equal, ["season"]);

			const { ctx } = this.$$;
			const props = options.props || {};
			if (ctx.season === undefined && !('season' in props)) {
	...
	const file$d = "src/components/Competition.svelte";
	...
	function create_each_block$7(ctx) {
		...
		return {
			c: function create() {
				...
				p = element("p");
				t = text(t_value);
				p.className = p_class_value = "choice " + (ctx.$selectedSeason == ctx.season.year ? "selected" : "") + " svelte-1qo6x4f";
				add_location(p, file$d, 3, 2, 105);
				add_location(li, file$d, 2, 1, 52);
				dispose = listen(li, "click", click_handler);
			},
			...
			p: function update(changed, new_ctx) {
				...
				if ((changed.$selectedSeason || changed.$seasons) && p_class_value !== (p_class_value = "choice " + (ctx.$selectedSeason == ctx.season.year ? "selected" : "") + " svelte-1qo6x4f")) {
					p.className = p_class_value;
				}
			},
	...
	function create_fragment$d(ctx) {
		...
		return {
			c: function create() {
				...
				div = element("div");
				season.$$.fragment.c();
				ul.className = "rows menu svelte-1qo6x4f";
				add_location(ul, file$d, 0, 0, 0);
				div.className = "competition svelte-1qo6x4f";
				add_location(div, file$d, 7, 0, 214);
			},
			...
			m: function mount(target, anchor) {
				...
				insert(target, div, anchor);
				mount_component(season, div, null);
				current = true;
			},
			...
			p: function update(changed, ctx) {
				...
				var season_changes = {};
				if (changed.$selectSeason) season_changes.season = ctx.$selectSeason;
				season.$set(season_changes);
			},

			i: function intro(local) {
				if (current) return;
				season.$$.fragment.i(local);

				current = true;
			},

			o: function outro(local) {
				season.$$.fragment.o(local);
				current = false;
			},
	...
	function instance$c($$self, $$props, $$invalidate) {
		let $seasons, $selectedSeason, $selectSeason;
		...
		validate_store(selectSeason, 'selectSeason');
		subscribe($$self, selectSeason, $$value => { $selectSeason = $$value; $$invalidate('$selectSeason', $selectSeason); });
	...
	const file$e = "src/components/Contact.svelte";
	...
	function create_fragment$f(ctx) {
		...
		var if_block3 = (ctx.$selectedPage == 'contact') && create_if_block$1(ctx);
		...
		return {
			c: function create() {
				...
				add_location(div0, file$f, 1, 1, 25);
				div1.className = "content-placement svelte-1k18n08";
				add_location(div1, file$f, 4, 1, 78);
